...
const STATE_FILE =
  process.env.HERMES_USAGE_STATE_FILE ||
  path.join(/* turbopackIgnore: true */ os.homedir(), ".ai-usage", "hermes-usage-state.json");
...
async function loadState(): Promise<HermesUsageState> {
  try {
    ...
    if (parsed.version !== 1 || !parsed.sessions || !parsed.events) {
...
export async function readHermesUsage(sinceDate: string): Promise<UsageEvent[]> {
  ...
  for (const row of rows) {
    ...
    const inputTokens = positiveInt(row.input_tokens);
    const outputTokens = positiveInt(row.output_tokens);
    const cacheReadTokens = positiveInt(row.cache_read_tokens);
