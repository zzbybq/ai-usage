...
import { emptyHermesUsageState, localDayKey, reconcileHermesUsage, type ObservedHermesSession } from "./hermes-state";

function at(day: number, hour: number): number { return new Date(2026, 6, day, hour).getTime(); }
function session(overrides: Partial<ObservedHermesSession> = {}): ObservedHermesSession {
  return { id: "session-1", model: "test-model", classifyMs: at(14, 19), inputTokens: 100, outputTokens: 20, cacheCreateTokens: 0, cacheReadTokens: 300, resolvedCostUSD: 1, ...overrides };
}
...
describe("Hermes cumulative usage ledger", () => {
  ...
  it("attributes only later positive deltas to the observation day", () => {
    ...
    reconcileHermesUsage(state, [session({ inputTokens: 160, outputTokens: 30, cacheReadTokens: 500, resolvedCostUSD: 1.5 })], at(15, 8));
    const today = Object.values(state.events).find((event) => localDayKey(new Date(event.timestamp).getTime()) === "2026-07-15");
